create table if not exists public.rental_items (
 ...
 status text not null default 'available' check (status in ('available','unavailable','hidden','maintenance')),
 quantity integer not null default 1 check (quantity > 0),
 daily_price_ore integer not null default 0,
 weekend_price_ore integer,
 weekly_price_ore integer,
 long_term_days integer,
 long_term_discount_percent numeric(5,2) not null default 0,
...
grant select,insert,update,delete on public.rental_items to service_role;
grant select,insert,update,delete on public.rental_blocks to service_role;
grant select,insert,update,delete on public.rental_bookings to service_role;
