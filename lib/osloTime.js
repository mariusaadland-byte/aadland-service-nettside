...
export function osloDateTimeLocal(value){
 if(!value)return "";
 const p=dateParts(value);
 if(!p)return "";
 return [
 ...
 ].join("-")+"T"+String(p.hour).padStart(2,"0")+":"+String(p.minute).padStart(2,"0");
}

export function osloLocalDateTimeToIso(value){
 const input=String(value||"").trim();
 ...
 if(hour>23||minute>59||!validDateInput(`${ys}-${ms}-${ds}`))return "";
 const wallUtc=Date.UTC(year,month-1,day,hour,minute,0,0);
 let utc=wallUtc-offsetMs(wallUtc);
 utc=wallUtc-offsetMs(utc);
 const result=new Date(utc);
 const p=dateParts(result);
 if(!p||p.year!==year||p.month!==month||p.day!==day||p.hour!==hour||p.minute!==minute)return "";
