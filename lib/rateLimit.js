...
function clientKey(req,scope){
 ...
 const secret=String(process.env.RATE_LIMIT_SECRET||process.env.SESSION_SECRET||"").trim();
 if(!secret)return null;
 return crypto.createHmac("sha256",secret).update(scope+":"+client).digest("hex");
}
