...
export function createCustomerPasswordResetToken({id,email,version},maxAgeSeconds=60*60){
...
export function verifyCustomerPasswordResetToken(token){
 try{
  ...
  if(data?.purpose!=="customer_password_reset"||!data?.id||!data?.email||!data?.version||!data?.exp||data.exp<Math.floor(Date.now()/1000))return null;
  return {id:String(data.id),email:String(data.email).trim().toLowerCase(),version:String(data.version)};
 }catch{
  return null;
 }
}
