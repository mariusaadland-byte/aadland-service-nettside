...
function sign(userId,version) {
  return crypto
    .createHmac("sha256", secret())
    .update("admin:"+userId+":"+version)
    .digest("hex");
}
...
function parseSession(value){
  ...
  const [userId,version,signature]=parts;
  if(!userId||!version||!signature||!/^[a-f0-9]{48}$/i.test(version))return null;
  const expected=sign(userId,version.toLowerCase());
  const a=Buffer.from(signature);
  const b=Buffer.from(expected);
  if(a.length!==b.length||!crypto.timingSafeEqual(a,b))return null;
  return {userId,version:version.toLowerCase()};
}
...
async function ensureSessionVersion(s,userId){
  ...
  const metadata={...(user.user_metadata||{}),[SESSION_VERSION_FIELD]:version};
  const {error:updateError}=await s.auth.admin.updateUserById(userId,{user_metadata:metadata});
...
export async function setAdminCookie(userId) {
  ...
  const value = `${userId}.${version}.${sign(userId,version)}`;
