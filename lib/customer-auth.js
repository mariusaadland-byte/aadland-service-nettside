...
export async function getCustomerUserId(){try{const c=await cookies(),v=c.get(COOKIE)?.value||"",dot=v.indexOf(".");if(dot<1)return null;const id=v.slice(0,dot),sig=v.slice(dot+1),expected=sign(id),a=Buffer.from(sig),b=Buffer.from(expected);return a.length===b.length&&crypto.timingSafeEqual(a,b)?id:null}catch(e){console.error("CUSTOMER SESSION",e);return null}}
