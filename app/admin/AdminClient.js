...
function ProjectEditor({project,reload,setError,close}){
 ...
 const imageEntry=x=>typeof x==="string"?{url:x,caption:""}:{url:x?.url||"",caption:x?.caption||""};
 const [v,setV]=useState({title:project?.title||"",category:project?.category||"",description:project?.description||"",imageUrls:(project?.imageUrls||[]).map(imageEntry),featured:project?.featured!==false,active:project?.active!==false,sortOrder:project?.sortOrder||0});
 const set=(k,x)=>setV({...v,[k]:x});
 const imageUrl=x=>typeof x==="string"?x:x?.url||"";
 function updateImage(i,changes){setV(x=>({...x,imageUrls:x.imageUrls.map((image,index)=>index===i?{...imageEntry(image),...changes}:imageEntry(image))}))}
 function moveImage(i,direction){setV(x=>{const next=x.imageUrls.map(imageEntry),target=i+direction;if(target<0||target>=next.length)return x;[next[i],next[target]]=[next[target],next[i]];return {...x,imageUrls:next}})}
 async function upload(files){setUploading(true);const images=[];for(const file of Array.from(files||[])){const fd=new FormData();fd.append("file",file);const r=await fetch("/api/admin/upload",{method:"POST",body:fd}),d=await r.json().catch(()=>({}));if(r.ok&&d.url)images.push({url:d.url,caption:""});else setError(d.error||"Et bilde kunne ikke lastes opp.");}setV(x=>({...x,imageUrls:[...x.imageUrls,...images]}));setUploading(false);}
 ...
 if(!editing&&project)return <article className="card">{project.imageUrls?.[0]&&<img src={imageUrl(project.imageUrls[0])} alt="" style={{width:"100%",height:200,objectFit:"cover",borderRadius:12}}/>}<div className="kicker">{project.category||"OPPDRAG"}</div><h3>{project.title}</h3><p>{project.description}</p><p className="muted">{project.active?"Publisert":"Skjult"} · {project.featured?"Vises på forsiden":"Ikke på forsiden"} · {project.imageUrls?.length||0} bilder</p><div style={{display:"flex",gap:10,flexWrap:"wrap"}}><button className="btn" onClick={()=>setEditing(true)}>Rediger</button>{project.active&&project.slug&&<a className="btn alt" href={"/prosjekter/"+project.slug} target="_blank" rel="noreferrer">Se kundeside</a>}<button className="btn alt" onClick={remove}>Slett</button></div></article>;
 ...
 <div className="field"><label>Bilder og bildetekst</label><p className="muted">Bildeteksten vises mellom bildene på kundesiden. Bruk den til å forklare hva som ble gjort i denne delen av oppdraget.</p>{v.imageUrls.map((image,i)=><div key={imageUrl(image)+i} className="projectImageEditor"><img src={imageUrl(image)} alt="" /><div className="projectImageEditorBody"><textarea rows="3" value={imageEntry(image).caption} onChange={e=>updateImage(i,{caption:e.target.value})} placeholder="F.eks. Vi bygget ny ramme, rettet opp underlaget og klargjorde for terrassebord …"/><div className="projectImageActions"><button type="button" className="btn alt" disabled={i===0} onClick={()=>moveImage(i,-1)}>↑ Flytt opp</button><button type="button" className="btn alt" disabled={i===v.imageUrls.length-1} onClick={()=>moveImage(i,1)}>↓ Flytt ned</button><button type="button" className="btn alt" onClick={()=>set("imageUrls",v.imageUrls.filter((_,x)=>x!==i))}>Fjern</button></div></div></div>)}<input type="file" multiple accept="image/jpeg,image/png,image/webp" disabled={uploading} onChange={e=>{upload(e.target.files);e.target.value=""}}/>{uploading&&<small>Laster opp bilder …</small>}</div>
