 ...
 <a key={service.id||service.slug} href={serviceHref(service)} onClick={()=>setMenuOpen(false)}>{service.title}</a>
  ...
  {siteSettings.showProjects!==false&&<section id="prosjekter" className="homeSection projects projectsSection"><div className="homeWrap">
   ...
   <div className="projectGrid">
    {apiProjects.length?apiProjects.filter(project=>project.featured!==false).slice(0,4).map((project,index)=><a href={"/prosjekter/"+project.slug} className={"projectTile projectCard "+(!project.imageUrls?.[0]?["projectDeck","projectBathroom","projectSlats","projectPlanters"][index%4]:"")} key={project.id} style={project.imageUrls?.[0]?{backgroundImage:"url("+project.imageUrls[0]+")",backgroundSize:"cover",backgroundPosition:"center"}:{}}><div className="projectCaption"><span>{(project.category||"OPPDRAG").toUpperCase()}</span><strong>{project.title}</strong>{project.description&&<p>{project.description}</p>}<em>Se prosjekt →</em></div></a>):<>
     <a href="/prosjekter" className="projectTile projectCard projectDeck"><div className="projectCaption"><span>UTEOMRÅDE</span><strong>Terrasse og levegg</strong><em>Se prosjekter →</em></div></a>
     <a href="/prosjekter" className="projectTile projectCard projectBathroom"><div className="projectCaption"><span>OPPUSSING</span><strong>Oppusset bad</strong><em>Se prosjekter →</em></div></a>
     <a href="/prosjekter" className="projectTile projectCard projectSlats"><div className="projectCaption"><span>INNVENDIG</span><strong>Spilevegg og trapp</strong><em>Se prosjekter →</em></div></a>
     <a href="/prosjekter" className="projectTile projectCard projectPlanters"><div className="projectCaption"><span>PÅ BESTILLING</span><strong>Plantekasser</strong><em>Se prosjekter →</em></div></a>
    </>}
   </div>
  </div></section>}
