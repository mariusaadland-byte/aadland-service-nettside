...
export default async function ProjectPage({params}){
 ...
 return <main className="projectDetailPage">
   ...
   {gallery.length?<div className="projectStoryList">
    {gallery.map((image,index)=><figure className={"projectStoryItem "+(index%2?"projectStoryItemReverse":"")} key={image.url+index}>
     ...
     <figcaption>
      ...
      {image.caption?<p>{image.caption}</p>:<p>Bilde fra prosjektet.</p>}
     </figcaption>
    </figure>)}
  ...
  <section className="projectDetailContact">
   <div>
    <span className="goldLabel">HAR DU NOE LIGNENDE I TANKENE?</span>
    <h2>Fortell oss om prosjektet ditt.</h2>
    <p>Vi tar gjerne en gratis og uforpliktende befaring og ser på hva som passer hos deg.</p>
   </div>
